...
export default class extends Component {
  render() {
    let color = "#ff6b6b";
    let size = this.props.size ? this.props.size : "2em";
    switch (this.props.icon) {
      case "heart-full":
        return this._heartFull(color, size);
      default:
        return this._heartEmpty(color, size);
    }
  }

  _heartEmpty(color, size) {
    return (
      <svg x="0px" y="0px" viewBox="0 0 477.534 477.534" className="classA">
        <style>{`.classA {
          fill:${color};
          height:${size};
         }`}</style>
